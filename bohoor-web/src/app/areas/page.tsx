...
export default async function AreasPage() {
  ...
  // Fallback images since locations might not have cover images from the backend yet
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-bold font-cairo text-gray-900 mb-4 tracking-tight">
            استكشف <span className="text-primary">المناطق</span>
          </h1>
          <p className="text-lg text-gray-500 max-w-2xl mx-auto font-cairo">
            تصفح العقارات والمشاريع المتاحة في أفضل المناطق والأحياء السكنية والتجارية.
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {locations.map((loc: any, i: number) => (
            <Link href={`/units?locationId=${loc.id}`} key={loc.id || i} className="relative h-72 rounded-3xl overflow-hidden group shadow-sm hover:shadow-xl transition-all block">
              <img 
                src={loc.image || fallbackImages[i % fallbackImages.length]} 
                alt={loc.name} 
                className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" 
              />
              <div className="absolute inset-0 bg-gradient-to-t from-gray-900 via-gray-900/30 to-transparent" />
              <div className="absolute bottom-6 right-6 text-white z-10 flex flex-col justify-end">
                <div className="flex items-center gap-2 mb-2">
                  <MapPinIcon className="w-6 h-6 text-primary" />
                  <h3 className="text-3xl font-bold font-cairo">{loc.name}</h3>
                </div>
                <p className="text-gray-300 font-medium mr-8">استكشف العقارات المتاحة &larr;</p>
              </div>
            </Link>
          ))}
