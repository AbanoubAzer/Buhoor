...
export default function UnitFilterSidebar({
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Developers only apply to DEVELOPER seller type (or ALL)
  ...
  return (
    <div className="bg-white rounded-3xl shadow-sm border border-gray-100 lg:sticky lg:top-24 flex flex-col max-h-[calc(100vh-7rem)] overflow-hidden">
      <form onSubmit={handleSubmit} className="flex flex-col h-full min-h-0">
          ...
          <div>
            ...
            <select 
              ...
              onChange={(e) => {
                ...
                setSelectedLoc(''); // Reset location when governorate changes
              }}
              className="w-full bg-gray-50 border border-gray-200 rounded-xl py-2 px-3 text-sm font-semibold focus:ring-2 focus:ring-primary outline-none"
            >
        ...
        <div className="pt-2 border-t border-gray-100">
          ...
          >
            <span>🌊 إطلالة بحرية / ع البحر فقط</span>
            <span className={`w-4 h-4 rounded-full border flex items-center justify-center ${seaView ? 'bg-primary border-primary text-white' : 'border-gray-400'}`}>
              {seaView && '✓'}
            </span>
          </button>
        </div>
      ...
      <div className="p-4 border-t border-gray-100 bg-white/95 backdrop-blur-sm shrink-0">
        <button type="submit" className="w-full bg-primary hover:bg-accent text-white font-bold py-3 rounded-xl transition shadow-lg shadow-primary/20 flex items-center justify-center gap-2">
